...
export class AuthService{
    ...
    return this.http.post('http://localhost:3000/employees/register', sub, options).map(res => res.json());
  }
  ...
  updateEmployee(id)
  {
     let headers = new Headers({ 'Content-Type': 'application/json' });
  let options = new RequestOptions({ headers: headers });
    return this.http.put('http://localhost:3000/employees/table/'+id,options)
            .map(res => res.json());  
  }

    getUser(){
        this.http.get('../src/app/assets/data/data.json').subscribe((res:Response)=>{
